...
public class LiveContributeViewHolder extends AbsLivePageViewHolder implements View.OnClickListener {
    ...
    public void loadData() {
        if (!mLoad) {
            mLoad = true;
            mWebView.loadUrl(HtmlConfig.LIVE_LIST + mLiveUid+"&trueuid="+AppConfig.getInstance().getUid());
            Log.e("zlg","url====="+HtmlConfig.LIVE_LIST + mLiveUid+"&trueuid="+AppConfig.getInstance().getUid());
        } else {
            mWebView.reload();
        }
    }
