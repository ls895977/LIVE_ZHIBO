...
public class RegisterActivity extends AbsActivity {
    ...
    protected void main() {
        ...
        mEditPhone.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (!TextUtils.isEmpty(s) && s.length() == 11 || s.length() == 9) {
                    mBtnCode.setEnabled(true);
                    mBtnCode.setTextColor(getResources().getColor(R.color.bt_pink));
                } else {
    ...
    private void getCode() {
       ...
       /* if (!ValidatePhoneUtil.validateMobileNumber(phoneNum)) {
        ...
        HttpUtil.getRegisterCode(countryCode.substring(1, countryCode.length()) + phoneNum, mContext, mGetCodeCallback);
    }
    ...
    private void register() {
       ...
       /* if (!ValidatePhoneUtil.validateMobileNumber(phoneNum)) {
        ...
        HttpUtil.register(countryCode.substring(1, countryCode.length()) +phoneNum, pwd, pwd2, code, mContext, new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (code == 0) {
                    login(phoneNum, pwd);
